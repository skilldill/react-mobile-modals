...
export const Modal: FC<PropsWithChildren<ModalProps>> = (props) => {
  ...
  const openDirectionClass = !!direction
    ? modalClasses[`modal-show-${direction}`]
    : modalClasses['modal-show-horizontal'];
  const classes = show
    ? `${modalClasses['modal']} ${openDirectionClass}`
    : modalClasses['modal'];
  ...
  const modalStyle: CSSProperties = useMemo(() => {
    ...
    const transformStyle =
      direction === 'vertical'
        ? {
            transform:
              stateStartY < MIN_END_TOUCH && stateTranslateY - stateStartY > 0
                ? `translateY(${stateTranslateY}px)`
                : 'none',
          }
        : {
            transform:
              stateStartX < MIN_END_TOUCH && stateTranslateX - stateStartX > 0
  ...
  const onTouchEnd =
    direction === 'vertical'
      ? handleTouchEndVertical
      : handleTouchEndHorizontal;
